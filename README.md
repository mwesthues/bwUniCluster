...
Once you have registered as a user for the bwUniCluster you will be assigned your own username, which is your current username at the University of Hohenheim, preceded by `ho_` (*e.g.* `ho_dummy`). You can then access the server via `ssh` as follows:
```
ssh ho_dummy@scc.kit.edu
```
